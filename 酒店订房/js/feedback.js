...
//			console.log("addFile:"+JSON.stringify(f));
	...
	mui('.icons').on('tap','i',function(){
	  	...
	  	if(this.classList.contains("icon-star1")){
	  		for(var i=0;i<index;i++){
				children[i].classList.remove('icon-star1');
				children[i].classList.add('icon-star1-copy');
	  		}
	  	}else{
	  		for (var i = index; i < 5; i++) {
	  			children[i].classList.add('icon-star1')
	  			children[i].classList.remove('icon-star1-copy')
	  		}
	  	}
	  	starIndex = index;
    });
