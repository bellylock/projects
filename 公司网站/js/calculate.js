$(function(){
    ...
    $(window).bind("scroll", function () {   
        ...
        var sd = (s - d) * 0.75;     //122
        if( stop >= sd  ){
            $(".total_fee1").css("position","relative");
            // $(".total_fee1").css("{position:absolute,left:0}");
        }else{
        ...
        // 3、在滚动条距离底端5%以内： scrollTop / (offsetHeight – clientHeight) >= 0.95
            // var height1 = $(document).get(0).offsetHeight;
            // var height2 = $(document).clientHeight();
            // var height3 = $(window).scrollTop();
            // var totalheight = parseFloat($(window).height()) + parseFloat($(window).scrollTop());
            // var h = $(document).height();
            // var a = $(document).scrollTop();
            // console.log( h + "---" + a + "--" + height1)
    ...
    $(window).bind("scroll", function () {   
        ...
        var sd = (s - d) * 0.87;     //122
        if( stop >= sd  ){
            $(".total_fee2").css("position","relative");
        }else{
    ...
    layui.use(['layer', 'form'], function(){
        ...
        form.on('select(area)', function(data){
            var text = data.elem[data.value].innerText;
            $(".cur_pla_vice").text(text);
            // $(".cur_high").text(context)
        });
        form.on('select(disk)', function(data){
            var context = data.elem[data.value].innerText; 
            $(".cur_high").text(context)
        });
    });
    ...
    $(".add_date").click(function(){
        ...
        if( num == 0){
            ...
            num--;
            $(".storage_bars").append(`
                <div class="storage_bar clearfix" index="${dataIndex}">
                    <div class="sys_contains sys_contains2" index="${dataIndex}">
                        <p>容量范围：20-32768</p>
                    </div>
                    <select>
                        <option>高效云盘</option>
                    ...
                    <input class="add_input" type="text" placeholder="20-32768" index="${dataIndex}" />
                    <span class="storage_gb">GB</span>
                    <span class="iops_num">1800</span>
                    <span class="iops">IOPS</span>
                    <div class="add_check clearfix" index="${dataIndex}">
                        <i class="rect" index="${dataIndex}"></i>
                        <span>加密</span>
                    </div>
                    <i class="iconfont icon-question add_ask"></i>
                    <span class="autoname">自动分配设置名</span>
                    <i class="iconfont add_cancel icon-x" index="${dataIndex}"></i>
                </div>
            `);
            ...
            $(".storage_bars").on('click','i.add_cancel', function(event){
                var index = $(this).attr("index");
                $($(".storage_bar")[index]).css("display","none")
            ...
            $(".storage_bars").on('input','.add_input', function(){
                this.value = this.value.replace(/\D/g,"");
            });
            $(".storage_bars").on('blur','.add_input', function(){
                var index = $(this).attr("index");
            ...
            $(".storage_bars").on('keydown','.add_input', function(event){
                var index = $(this).attr("index");
                var value = $($(".add_input")[index]).val();
                if( event.keyCode == 38){
                    if( value >= 32768){
            ...
            $(".storage_bars").on('mouseenter mouseleave','.add_input', function(event){
                var index = $(this).attr("index");
                if( event.type == "mouseenter"){
            ...
            $(".storage_bars").on('click','.add_check', function(){ 
    ...
    $(".purchase_spebox2 span").click(function(){
        $(".purchase_spebox2 span").removeClass("purchase_cover");
        $(this).addClass("purchase_cover")
    });
    ...
    $(".body_text tbody tr").click(function(){
        var index = $(this).attr("index");
        $(".td_pick").removeClass("td_spot");
        $($(".td_pick")[index]).addClass("td_spot")
    });

    $(".body_text tbody tr").click(function(){
        var normstext = $(".td_spot").parents(".td1").siblings(".td2").text();
        var norms_sty_text = $(".td_spot").siblings("span").text();
        var norms_he_text = $(".td_spot").parents("td").siblings(".td3").find("span").text();
        var norms_ne_text = $(".td_spot").parents("td").siblings(".td4").find("span").text();
